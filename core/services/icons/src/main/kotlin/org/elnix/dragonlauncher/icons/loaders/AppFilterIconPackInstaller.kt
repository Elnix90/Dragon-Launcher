...
import org.elnix.dragonlauncher.icons.IconBack
import org.elnix.dragonlauncher.icons.IconMask
...
internal class AppFilterIconPackInstaller(
    ...
    override suspend fun IconPackInstallerScope.buildIconPack(iconPack: IconPack) {
        withContext(Dispatchers.IO) {
            try {
                ...
                logD(ICONS_TAG) { "Icon pack ${iconPack.packageName} has been installed successfully" }
            } catch (e: PackageManager.NameNotFoundException) {
                logE(ICONS_TAG, e) { "Could not install icon pack ${iconPack.packageName}: package not found." }
            } catch (e: XmlPullParserException) {
                logE(ICONS_TAG, e) { "Error building icon pack" }
            }
        }
    }
    ...
    private suspend fun IconPackInstallerScope.parseDrawableXml(
        ...
        val res = ctx.packageManager.getResourcesForApplication(iconPack.packageName)
    ...
    private fun getAppfilterParser(packageName: String): ClosableXmlParser? {
        val res = ctx.packageManager.getResourcesForApplication(packageName)
    ...
    override fun getInstalledIconPacks(): List<IconPack> {
        ...
        for (intent in intents) {
            packs.addAll(
                pm.queryIntentActivities(intent, 0).map { IconPack(ctx, it, true) }
            )
        }
