...
public object ExtensionManager {
    ...
    public suspend fun isExtensionInstalled(ctx: Context, packageNameOrId: String): Boolean {
        ...
        return checkExtensionInstalled(ctx, packageNameOrId, disableSigCheck)
    }

    private fun checkExtensionInstalled(ctx: Context, packageNameOrId: String, disableSigCheck: Boolean): Boolean {
        try {
            // Signature check
            if (!disableSigCheck) {
