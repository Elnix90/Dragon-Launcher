 ...
 *
 * Covers:
 * - [Float.toRadians]: degrees -> radians conversion
...
class FloatKtxTest {
    ...
    fun `0 degrees converts to 0 radians`() {
        assertEquals(0f, 0f.toRadians().toFloat(), EPSILON)
    }

    @Test
    fun `90 degrees converts to PI over 2`() {
        val expected = (PI / 2.0).toFloat()
        assertEquals(expected, 90f.toRadians().toFloat(), EPSILON)
    }

    @Test
    fun `180 degrees converts to PI`() {
        assertEquals(PI.toFloat(), 180f.toRadians().toFloat(), EPSILON)
    }

    @Test
    fun `360 degrees converts to 2 PI`() {
        val expected = (2.0 * PI).toFloat()
        assertEquals(expected, 360f.toRadians().toFloat(), EPSILON)
    }

    @Test
    fun `45 degrees converts to PI over 4`() {
        val expected = (PI / 4.0).toFloat()
        assertEquals(expected, 45f.toRadians().toFloat(), EPSILON)
    }

    @Test
    fun `270 degrees converts to 3 PI over 2`() {
        val expected = (3.0 * PI / 2.0).toFloat()
        assertEquals(expected, 270f.toRadians().toFloat(), EPSILON)
    }
}
